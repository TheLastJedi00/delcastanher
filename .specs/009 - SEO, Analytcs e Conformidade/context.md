...
4. **Nenhum evento sai antes do consentimento; o que acontece antes vai para fila.**
   ...
   > **Ratificada pela Spec 010 (decisão 9):** o player do Mux entrou com a telemetria (Mux Data) desabilitada e sem env key, justamente porque a área logada não tem banner próprio — terceiro nenhum grava identificador sem base legal declarada.
...
12. **Nenhuma dependência nova entra no bundle.**
    ...
    > **DEPRECATED como regra geral pela Spec 010 (decisão 8):** `@mux/mux-player` entrou no `front/` porque streaming com playback assinado não se implementa com `<iframe>`. A regra continua valendo como critério — nenhuma lib entra sem necessidade que o framework não cubra — e o pacote é carregado apenas no browser, na área logada, fora das rotas prerenderizadas.
