...
- [x] **Task 6.1:** Revisar responsividade da tabela no mobile (rolagem horizontal ou empilhamento das colunas) e a acessibilidade: `scope` nos cabeçalhos, estado de ordenação anunciado por `aria-sort`, `aria-live` nos resultados da busca e rótulos dos botões de ação dizendo de qual aluno se trata.
- [x] **Task 6.2:** Conferir que nenhuma rota nova responde a papel `aluno` nem a requisição sem token, e que nenhum dado de outro usuário aparece em resposta fora de `/admin/*`.
- [x] **Task 6.3:** Conferir que o `RolesGuard` continua decidindo pelo claim do token e que a coluna `role` não é lida em nenhum caminho de autorização (decisão 3).
- [x] **Task 6.4:** Rodar `npm test` e `npm run build` no `front/` e `npm test` no `api/`, corrigindo regressões.
